...
import { Story, Profile, Language, Category, StoryLength, ReadingLevel } from '../types';
import { AUTHOR_STYLES, HINDI_AUTHOR_STYLES } from '../constants';
import { ArrowLeft, Quote, Globe, MapPin, Feather, Scroll, Flag, Image as ImageIcon, RefreshCw, Printer } from 'lucide-react';
...
interface StoryViewProps {
  ...
  currentEnglishStyleId: string;
  currentHindiStyleId: string;
...
export const StoryView: React.FC<StoryViewProps> = ({
  story, profile, category, onBack, displayLanguage, loadingImages,
  storyLength, currentEnglishStyleId, currentHindiStyleId, onRegenerate,
  readingLevel, onReadingLevelChange,
}) => {
  const [reportOpen, setReportOpen] = useState(false);
  const [imgError, setImgError] = useState(false);
  const [mapImgError, setMapImgError] = useState(false);
  const [localEnglishStyleId, setLocalEnglishStyleId] = useState(currentEnglishStyleId);
  const [localHindiStyleId, setLocalHindiStyleId] = useState(currentHindiStyleId);
  ...
  const handleRegenerateClick = () => {
    onRegenerate(localEnglishStyleId, localHindiStyleId, storyLength);
  };

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 animate-fadeIn">
      <button onClick={onBack} className="group mb-8 flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-ink transition-colors no-print">
      ...
      <article className="bg-white rounded-[2rem] shadow-2xl shadow-gray-200/50 overflow-hidden border border-parchment-dark relative">
        ...
        <div className="relative h-[40vh] min-h-[300px] md:h-[500px] bg-gray-100 overflow-hidden">
          {loadingImages ? (
            ...
            <img src={story.generatedImageUrl} alt={story.illustrationPrompt} onError={() => setImgError(true)} className="w-full h-full object-cover animate-fadeIn" />
          ) : (
        ...
        <div className="p-6 sm:p-10 md:p-14 max-w-4xl mx-auto">
          ...
          {/* Toolbar: length toggle + age toggle + print */}
          <div className="no-print flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-6 border-b border-gray-100">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2">
                ...
                {(['short', 'medium', 'long'] as StoryLength[]).map(l => (
                  <button
                    key={l}
                    onClick={() => onRegenerate(localEnglishStyleId, localHindiStyleId, l)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all border ${storyLength === l ? 'bg-ink text-white border-ink' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'}`}
                  >
                    {LENGTH_LABELS[l]}
                  </button>
                ))}
          ...
          {/* Values + Storyteller attribution */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                {profile.values.map(v => (
                  <span key={v} className="px-3 py-1 bg-blue-50 text-ocean text-xs sm:text-sm font-semibold rounded-full border border-blue-100 tracking-wide">{v}</span>
                ))}
              </div>
          ...
          {/* Style Picker + Regenerate — no-print */}
          <div className="no-print bg-gray-50 rounded-2xl p-5 border border-gray-100 mb-10 flex flex-col sm:flex-row gap-4 items-end">
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">English Style</label>
                <select
                  value={localEnglishStyleId}
                  onChange={e => setLocalEnglishStyleId(e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-gold/30"
                >
                  {AUTHOR_STYLES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Hindi Style</label>
                <select
                  value={localHindiStyleId}
                  onChange={e => setLocalHindiStyleId(e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-gold/30"
                >
                  {HINDI_AUTHOR_STYLES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
            </div>
            <button
              onClick={handleRegenerateClick}
              className="flex items-center gap-2 px-5 py-2.5 bg-gold text-white rounded-xl font-bold text-sm hover:bg-gold/90 transition-all shadow-sm shrink-0"
            >
          ...
          <div className="bg-parchment rounded-2xl p-8 sm:p-10 border border-gold/20 relative mb-14 shadow-sm">
            ...
            <div className="relative z-10 text-center max-w-2xl mx-auto">
              ...
              <p className="text-2xl sm:text-3xl font-serif text-ink italic leading-snug">"{content.valueReflection}"</p>
            </div>
          </div>
          ...
          {story.geography && (
            <div className="bg-blue-50/50 rounded-3xl p-8 sm:p-10 border border-blue-100 mb-14">
              <div className="flex items-center gap-3 mb-8">
                <div className="p-2 bg-white rounded-full shadow-sm text-ocean"><Globe size={24} /></div>
              ...
              <div className="flex flex-col md:flex-row gap-10 items-center">
                <div className="w-full md:w-1/2">
                  <div className="aspect-square bg-white rounded-2xl shadow-md p-3 rotate-1 transform hover:rotate-0 transition-transform duration-500 border border-gray-100 relative overflow-hidden">
                    {loadingImages ? (
                      <div className="w-full h-full flex items-center justify-center bg-gray-50 animate-pulse rounded-xl"><MapPin size={32} className="text-gray-300 mb-2" /></div>
                    ) : (story.generatedMapUrl && !mapImgError) ? (
                      <img src={story.generatedMapUrl} alt={`Map of ${story.geography.countryName}`} onError={() => setMapImgError(true)} className="w-full h-full object-cover rounded-xl animate-fadeIn" />
                    ) : (
                  ...
                  <p className="text-center text-xs text-gray-400 mt-4 font-bold uppercase tracking-[0.15em]">{story.geography.countryName}</p>
                </div>
                <div className="w-full md:w-1/2 space-y-4">
                  <h4 className="font-bold text-ocean text-lg mb-2">Did you know?</h4>
                  <p className="text-gray-700 leading-relaxed text-lg"><Typewriter text={story.geography.funFact} speed={20} /></p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="bg-ink text-parchment rounded-3xl p-8 sm:p-10 border border-gray-800 relative overflow-hidden mb-10">
            <div className="absolute top-0 right-0 p-12 opacity-5 rotate-12 pointer-events-none"><Scroll size={180} /></div>
            <div className="relative z-10">
              ...
              <div className="flex flex-col md:flex-row gap-10">
                <div className="flex-1 space-y-4">
                  ...
                  <p className="text-gray-300 leading-relaxed italic text-lg opacity-90">"{authorInfo.description}"</p>
                </div>
      ...
      <ReportDialog isOpen={reportOpen} onClose={() => setReportOpen(false)} contentTitle={content.title} />
    </div>
  );
};
